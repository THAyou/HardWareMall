...
namespace Dal.Base
{
    ...
    public abstract class BaseDal
    {
        ...
        public BaseDal(ESqlConnType eSqlConnType)
        {
            switch (eSqlConnType)
            {
                case ESqlConnType.HardwareMall:
                    sqlConnectionString = ConfigHelper.Configuration["SqlServer:SqlServerConnection"].ToString();  //默认读数据库链接
                    sqlConnectionStringWrite = ConfigHelper.Configuration["SqlServer:SqlServerConnection"].ToString();  //默认写数据库链接
                    break;
                //当需要链接多个数据库的时候启用
                ...
                //    sqlConnectionStringWrite = ConfigurationManager.ConnectionStrings["ConnectionStringPdf"].ConnectionString; 
                //    break;
                default:
                    sqlConnectionString = ConfigHelper.Configuration["SqlServer:SqlServerConnection"].ToString();  //默认读数据库链接
                    sqlConnectionStringWrite = ConfigHelper.Configuration["SqlServer:SqlServerConnection"].ToString();  //默认写数据库链接
